...
struct ValidID ValidID; //stores current Valid user ID parameters
...
//To extract year from userID -- For example, userID:2021is404 year:2021
int extYear(char userID[15])
{
    int year=0;
    char tmp;
    for(int i=0;i<4;i++){
        ...
        while(1)
        {
            ...
            switch(input)
            {
                case '1':
                    ...
                    break;
                case '2':
                    getElectndataFromFile();
                    break;
                case '3':
                    printf("\nEnter user ID to delete its vote: ");
                    scanf("%s",inputID);
                    delillegalVote(inputID);
                    break;
                case '4':
...
int isValid(char userID[15])
{
    ...
    int inputedYear=extYear(userID);
    int inputedRollNo = RollNo(userID);
    
    if(inputedYear!=ValidID.year || BrnCode(userID)!=1 || inputedRollNo>ValidID.totalVoters)
        return 0;
